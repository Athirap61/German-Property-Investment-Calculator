...
class Lohnsteuer2024:
    ...
    def MAIN(self):
        ...
        self.MRE4()
        self.MRE4ABZ()
        self.ZRE4VPM = self.ZRE4VP
        self.SCHLEIFZ = 1
        self.MBERECH()
        self.SCHLEIFZ = 2
        self.W1STKL5 = BigDecimal.valueOf(13432)
        self.GFB = BigDecimal.valueOf(11784)
        self.SOLZFREI = BigDecimal.valueOf(18130)
        self.ZRE4VP = self.ZRE4VPM
        self.MBERECH()
        self.MLST1224()
        self.MSONST()
        self.MVMT()
    ...
    def MPARA(self):
        ...
        self.KVSATZAN = self.KVZ.divide(Lohnsteuer2024.ZAHL2).divide(Lohnsteuer2024.ZAHL100).add(BigDecimal.valueOf(0.07))
    ...
    def MRE4JL(self):
        ...
        """
        if self.LZZ == 1:
            self.ZRE4J = self.RE4.divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.VBEZ.divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLFREIB = self.LZZFREIB.divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLHINZU = self.LZZHINZU.divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        else:
            if self.LZZ == 2:
                self.ZRE4J = self.RE4.multiply(Lohnsteuer2024.ZAHL12).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.ZVBEZJ = self.VBEZ.multiply(Lohnsteuer2024.ZAHL12).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLFREIB = self.LZZFREIB.multiply(Lohnsteuer2024.ZAHL12).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLHINZU = self.LZZHINZU.multiply(Lohnsteuer2024.ZAHL12).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ZRE4J = self.RE4.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                else:
                    self.ZRE4J = self.RE4.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lohnsteuer2024.ZAHL360).divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        if self.af == 0:
            self.f = 1
    ...
    def MRE4(self):
        ...
        if self.ZVBEZJ.compareTo(BigDecimal.ZERO) == 0:
            ...
            if self.LZZ == 1:
                ...
                self.HFVB = Lohnsteuer2024.TAB2[self.J].divide(Lohnsteuer2024.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB))
                self.FVBZ = Lohnsteuer2024.TAB3[self.J].divide(Lohnsteuer2024.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB)).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.VBEZB = self.VBEZM.multiply(Lohnsteuer2024.ZAHL12).add(self.VBEZS).setScale(2, BigDecimal.ROUND_DOWN)
                self.HFVB = Lohnsteuer2024.TAB2[self.J]
                self.FVBZ = Lohnsteuer2024.TAB3[self.J]
            self.FVB = self.VBEZB.multiply(Lohnsteuer2024.TAB1[self.J]).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_UP)
            if self.FVB.compareTo(self.HFVB) == 1:
                self.FVB = self.HFVB
            if self.FVB.compareTo(self.ZVBEZJ) == 1:
                self.FVB = self.ZVBEZJ
            self.FVBSO = self.FVB.add(self.VBEZBSO.multiply(Lohnsteuer2024.TAB1[self.J]).divide(Lohnsteuer2024.ZAHL100)).setScale(2, BigDecimal.ROUND_UP)
            if self.FVBSO.compareTo(Lohnsteuer2024.TAB2[self.J]) == 1:
                self.FVBSO = Lohnsteuer2024.TAB2[self.J]
            self.HFVBZSO = self.VBEZB.add(self.VBEZBSO).divide(Lohnsteuer2024.ZAHL100).subtract(self.FVBSO).setScale(2, BigDecimal.ROUND_DOWN)
            self.FVBZSO = self.FVBZ.add(self.VBEZBSO.divide(Lohnsteuer2024.ZAHL100)).setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(self.HFVBZSO) == 1:
                self.FVBZSO = self.HFVBZSO.setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(Lohnsteuer2024.TAB3[self.J]) == 1:
                self.FVBZSO = Lohnsteuer2024.TAB3[self.J]
            self.HFVBZ = self.VBEZB.divide(Lohnsteuer2024.ZAHL100).subtract(self.FVB).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MRE4ALTE(self):
        ...
        if self.ALTER1 == 0:
            ...
            self.ALTE = self.BMG.multiply(Lohnsteuer2024.TAB4[self.K]).setScale(0, BigDecimal.ROUND_UP)
            self.HBALTE = Lohnsteuer2024.TAB5[self.K]
    ...
    def MRE4ABZ(self):
        ...
        if self.KENNVMT == 2:
            self.ZRE4VP = self.ZRE4VP.subtract(self.ENTSCH.divide(Lohnsteuer2024.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MBERECH(self):
        ...
        if self.SCHLEIFZ == 1:
            ...
            self.MZTABFBN()
        self.VFRB = self.ANP.add(self.FVB.add(self.FVBZ)).multiply(Lohnsteuer2024.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
        self.MLSTJAHR()
        self.WVFRB = self.ZVE.subtract(self.GFB).multiply(Lohnsteuer2024.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def MZTABFBN(self):
        ...
        if self.STKL == 1:
            self.SAP = BigDecimal.valueOf(36)
            self.KFB = self.ZKF.multiply(BigDecimal.valueOf(9540)).setScale(0, BigDecimal.ROUND_DOWN)
        else:
            if self.STKL == 2:
                self.EFA = BigDecimal.valueOf(4260)
                self.SAP = BigDecimal.valueOf(36)
                ...
                if self.STKL == 3:
                    ...
                    if self.STKL == 4:
                        self.SAP = BigDecimal.valueOf(36)
                        self.KFB = self.ZKF.multiply(BigDecimal.valueOf(4770)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        if self.STKL == 5:
                            self.SAP = BigDecimal.valueOf(36)
                            self.KFB = BigDecimal.ZERO
    ...
    def MLSTJAHR(self):
        ...
        if self.KENNVMT != 1:
            ...
            self.ZVE = self.ZRE4.subtract(self.ZTABFB).subtract(self.VSP).subtract(self.VMT.divide(Lohnsteuer2024.ZAHL100)).subtract(self.VKAPA.divide(Lohnsteuer2024.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
            if self.ZVE.compareTo(BigDecimal.ZERO) == -1:
                self.ZVE = self.ZVE.add(self.VMT.divide(Lohnsteuer2024.ZAHL100)).add(self.VKAPA.divide(Lohnsteuer2024.ZAHL100)).divide(Lohnsteuer2024.ZAHL5).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.multiply(Lohnsteuer2024.ZAHL5).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.UPMLST()
                self.STOVMT = self.ST
                self.ZVE = self.ZVE.add(self.VMT.add(self.VKAPA).divide(Lohnsteuer2024.ZAHL500)).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.subtract(self.STOVMT).multiply(Lohnsteuer2024.ZAHL5).add(self.STOVMT).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def UPVKV(self):
        ...
        if self.PKV > 0:
            if self.VSP2.compareTo(self.VSP3) == 1:
                self.VKV = self.VSP2.multiply(Lohnsteuer2024.ZAHL100)
            else:
                self.VKV = self.VSP3.multiply(Lohnsteuer2024.ZAHL100)
        else:
    ...
    def UPLSTLZZ(self):
        ...
        PAP Seite 27
        """
        self.JW = self.LSTJAHR.multiply(Lohnsteuer2024.ZAHL100)
        if self.SCHLEIFZ == 1:
            self.JWLSTA = self.JW
        else:
            self.JWLSTN = self.JW
    ...
    def UPMLST(self):
        ...
        """
        if self.ZVE.compareTo(Lohnsteuer2024.ZAHL1) == -1:
    ...
    def MVSP(self):
        ...
        if self.PKV > 0:
            if self.STKL == 6:
                ...
                self.VSP3 = self.PKPV.multiply(Lohnsteuer2024.ZAHL12).divide(Lohnsteuer2024.ZAHL100)
                if self.PKV == 2:
    ...
    def UP5_6(self):
        ...
        if self.SCHLEIFZ == 1:
            ...
            self.UPTAB24N()
        self.ST2 = self.ST
        self.DIFF = self.ST1.subtract(self.ST2).multiply(Lohnsteuer2024.ZAHL2)
    ...
    def MSOLZ(self):
        ...
        if self.JBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZJ = self.JBMG.multiply(BigDecimal.valueOf(5.5)).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.SOLZMIN = self.JBMG.subtract(self.SOLZFREI).multiply(BigDecimal.valueOf(11.9)).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.SOLZMIN.compareTo(self.SOLZJ) == -1:
                self.SOLZJ = self.SOLZMIN
            self.JW = self.SOLZJ.multiply(Lohnsteuer2024.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            if self.SCHLEIFZ == 1:
                self.JWSOLZA = self.JW
            else:
                self.JWSOLZN = self.JW
            self.UPANTEIL()
            self.SOLZLZZ = self.ANTEIL1
        else:
            self.SOLZLZZ = BigDecimal.ZERO
        if self.R > 0:
            self.JW = self.JBMG.multiply(Lohnsteuer2024.ZAHL100)
            if self.SCHLEIFZ == 1:
                self.JWBKA = self.JW
            else:
                self.JWBKN = self.JW
    ...
    def MLST1224(self):
        ...
        """
        if self.LZZ > 1:
            self.JW = self.JWLSTN.subtract(BigDecimal.valueOf(11).multiply(self.JWLSTA.subtract(self.JWLSTN)))
            if self.JW.compareTo(BigDecimal.ZERO) < 0:
                self.ANTEIL1 = BigDecimal.ZERO
            else:
                self.UPANTEIL()
            self.LSTLZZ = self.ANTEIL1
            self.JW = self.JWSOLZN.subtract(BigDecimal.valueOf(11).multiply(self.JWSOLZA.subtract(self.JWSOLZN)))
            if self.JW.compareTo(BigDecimal.ZERO) < 0:
                self.ANTEIL1 = BigDecimal.ZERO
            else:
                self.UPANTEIL()
            self.SOLZLZZ = self.ANTEIL1
            self.JW = self.JWBKN.subtract(BigDecimal.valueOf(11).multiply(self.JWBKA.subtract(self.JWBKN)))
            if self.JW.compareTo(BigDecimal.ZERO) < 0:
                self.ANTEIL1 = BigDecimal.ZERO
            else:
                self.UPANTEIL()
    ...
    def UPANTEIL(self):
        ...
        if self.LZZ == 1:
            ...
            if self.LZZ == 2:
                self.ANTEIL1 = self.JW.divide(Lohnsteuer2024.ZAHL12, 0, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ANTEIL1 = self.JW.multiply(Lohnsteuer2024.ZAHL7).divide(Lohnsteuer2024.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                else:
                    self.ANTEIL1 = self.JW.divide(Lohnsteuer2024.ZAHL360, 0, BigDecimal.ROUND_DOWN)
    ...
    def MSONST(self):
        ...
        if self.SONSTB.compareTo(BigDecimal.ZERO) == 0 and self.MBV.compareTo(BigDecimal.ZERO) == 0:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.VBEZBSO = self.STERBE
            self.MRE4SONST()
            self.MLSTJAHR()
            self.WVFRBM = self.ZVE.subtract(self.GFB).multiply(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.WVFRBM.compareTo(BigDecimal.ZERO) == -1:
                self.WVFRBM = BigDecimal.ZERO
            self.UPVKV()
            self.VKVSONST = self.VKV.subtract(self.VKVSONST)
            self.LSTSO = self.ST.multiply(Lohnsteuer2024.ZAHL100)
            self.STS = self.LSTSO.subtract(self.LSTOSO).multiply(BigDecimal.valueOf(self.f)).divide(Lohnsteuer2024.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lohnsteuer2024.ZAHL100)
            self.STSMIN()
    ...
    def STSMIN(self):
        ...
        if self.STS.compareTo(BigDecimal.ZERO) == -1:
            if self.MBV.compareTo(BigDecimal.ZERO) == 0:
                ...
                self.SOLZLZZ = self.SOLZLZZ.add(self.STS.multiply(BigDecimal.valueOf(5.5).divide(Lohnsteuer2024.ZAHL100))).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def MSOLZSTS(self):
        ...
        if self.SOLZSBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZS = self.STS.multiply(BigDecimal.valueOf(5.5)).divide(Lohnsteuer2024.ZAHL100, 0, BigDecimal.ROUND_DOWN)
        else:
            self.SOLZS = BigDecimal.ZERO
    ...
    def MVMT(self):
        ...
        if self.VMT.add(self.VKAPA).compareTo(BigDecimal.ZERO) == 1:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).add(self.VMT).add(self.VKAPA).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).add(self.VKAPA).divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.KENNVMT = 2
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LST3 = self.ST.multiply(Lohnsteuer2024.ZAHL100)
            self.MRE4ABZ()
            self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lohnsteuer2024.ZAHL100)).subtract(self.SONSTENT.divide(Lohnsteuer2024.ZAHL100))
            self.KENNVMT = 1
            self.MLSTJAHR()
            self.LST2 = self.ST.multiply(Lohnsteuer2024.ZAHL100)
            ...
            if self.STV.compareTo(BigDecimal.ZERO) == -1:
                ...
                self.STV = self.STV.multiply(BigDecimal.valueOf(self.f)).divide(Lohnsteuer2024.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lohnsteuer2024.ZAHL100)
            self.SOLZVBMG = self.STV.divide(Lohnsteuer2024.ZAHL100, 0, BigDecimal.ROUND_DOWN).add(self.JBMG)
            if self.SOLZVBMG.compareTo(self.SOLZFREI) == 1:
                self.SOLZV = self.STV.multiply(BigDecimal.valueOf(5.5)).divide(Lohnsteuer2024.ZAHL100, 0, BigDecimal.ROUND_DOWN)
            else:
    ...
    def MOSONST(self):
        ...
        """
        self.ZRE4J = self.JRE4.divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.ZVBEZJ = self.JVBEZ.divide(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.JLFREIB = self.JFREIB.divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.JLHINZU = self.JHINZU.divide(Lohnsteuer2024.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.MRE4()
        self.MRE4ABZ()
        self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lohnsteuer2024.ZAHL100))
        self.MZTABFBN()
        self.VFRBS1 = self.ANP.add(self.FVB.add(self.FVBZ)).multiply(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.MLSTJAHR()
        self.WVFRBO = self.ZVE.subtract(self.GFB).multiply(Lohnsteuer2024.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        if self.WVFRBO.compareTo(BigDecimal.ZERO) == -1:
            self.WVFRBO = BigDecimal.ZERO
        self.LSTOSO = self.ST.multiply(Lohnsteuer2024.ZAHL100)
    ...
    def MRE4SONST(self):
        ...
        self.FVB = self.FVBSO
        self.MRE4ABZ()
        self.ZRE4VP = self.ZRE4VP.add(self.MBV.divide(Lohnsteuer2024.ZAHL100)).subtract(self.JRE4ENT.divide(Lohnsteuer2024.ZAHL100)).subtract(self.SONSTENT.divide(Lohnsteuer2024.ZAHL100))
        self.FVBZ = self.FVBZSO
        self.MZTABFBN()
        self.VFRBS2 = self.ANP.add(self.FVB).add(self.FVBZ).multiply(Lohnsteuer2024.ZAHL100).subtract(self.VFRBS1)

    def UPTAB24A(self):
        """
        Tarifliche Einkommensteuer §32a EStG vor Gesetzesänderung, PAP Seite 42
        """
        if self.X.compareTo(self.GFB.add(Lohnsteuer2024.ZAHL1)) == -1:
            self.ST = BigDecimal.ZERO
        else:
            if self.X.compareTo(BigDecimal.valueOf(17006)) == -1:
                self.Y = self.X.subtract(self.GFB).divide(Lohnsteuer2024.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                ...
                if self.X.compareTo(BigDecimal.valueOf(66761)) == -1:
                    self.Y = self.X.subtract(BigDecimal.valueOf(17005)).divide(Lohnsteuer2024.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
    ...
    def UPTAB24N(self):
        ...
        if self.X.compareTo(self.GFB.add(Lohnsteuer2024.ZAHL1)) == -1:
            ...
            if self.X.compareTo(BigDecimal.valueOf(17006)) == -1:
                self.Y = self.X.subtract(self.GFB).divide(Lohnsteuer2024.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                self.RW = self.Y.multiply(BigDecimal.valueOf(954.8))
                self.RW = self.RW.add(BigDecimal.valueOf(1400))
                self.ST = self.RW.multiply(self.Y).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                if self.X.compareTo(BigDecimal.valueOf(66761)) == -1:
                    ...
                    self.RW = self.RW.add(BigDecimal.valueOf(2397))
                    self.RW = self.RW.multiply(self.Y)
                    self.ST = self.RW.add(BigDecimal.valueOf(991.21)).setScale(0, BigDecimal.ROUND_DOWN)
                else:
                    if self.X.compareTo(BigDecimal.valueOf(277826)) == -1:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.42)).subtract(BigDecimal.valueOf(10636.31)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.45)).subtract(BigDecimal.valueOf(18971.06)).setScale(0, BigDecimal.ROUND_DOWN)
        self.ST = self.ST.multiply(BigDecimal.valueOf(self.KZTAB))
